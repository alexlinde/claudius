 ...
 *
 * For SPI panel IO esp_lvgl_port doesn't install a flush_wait_cb, so LVGL
 * busy-spins in wait_for_flushing() until the panel IO `on_color_trans_done`
 * ISR calls `lv_disp_flush_ready`. If the ISR ever stops firing the busy
 * loop is permanent and starves IDLE0 -> task watchdog reset. We mark
 * each FLUSH_WAIT_START / FLUSH_WAIT_FINISH so an off-thread observer
 * can spot a wait that has been outstanding too long.
 *
